...
def generate_script(state: AppState) -> AppState:
    """Generate the final script with error handling"""
    query = state["query"]
    docs = state.get("docs", [])
    errors = state.get("errors", [])
    tone = state.get("tone", "neutral")
    format_type = state.get("format_type", "default")
    voice_tone = state.get("voice_tone", "calm")
    script_length = state.get("script_length", 800)
    
    if not docs:
        fallback_script = f"Unable to generate a script for '{query}' due to missing reference materials."
        errors.append("Script generation failed: No documents available")
        return {"script": fallback_script, "errors": errors}
    ...
    try:
        ...
        for doc in docs:
            ...
            context += doc.page_content[:1500] + "\n\n"
        
        # Calculate length range (±10% of target)
        min_length = max(300, int(script_length * 0.9))
        max_length = min(2000, int(script_length * 1.1))
        
        prompt = f"""
        Write a {tone.lower()} {format_type.lower()} YouTube video script with a {voice_tone.lower()} voice tone.
        
        Make it:
        - Storytelling-focused with a narrative arc
        - Engaging and human-like in delivery
        - {min_length}-{max_length} words in length
        - Ready to read aloud (no headings, bullet points, or formatting that's only for reading)
        
        Research Context:
        {context[:4000]}  # Limit total context
        ...
        try:
            ...
            logger.warning(f"Vertex AI script generation failed: {vertex_error}, falling back to Google Generative AI")
        ...
        try:
            ...
            st.session_state.graph_data = knowledge_graph.visualize_graph()
            st.session_state.script_generated = True
            logger.info("Knowledge graph data pre-generated successfully")
        except Exception as kg_error:
            logger.error(f"Error pre-generating knowledge graph: {kg_error}")
            st.session_state.script_generated = False
            
        return {"script": script_content, "errors": errors}
    except Exception as e:
        logger.error(f"Script generation failed: {e}")
        fallback_script = f"Unable to generate a complete script for '{query}'. Please try again with a modified prompt."
        errors.append(f"Script generation error: {str(e)}")
        return {"script": fallback_script, "errors": errors}
...
def main():
    ...
    with tab2:
        ...
        elif 'script_generated' in st.session_state and st.session_state.script_generated:
            st.success("✅ Script generated, knowledge graph should be available")
        ...
        if st.button("View Knowledge Graph", type="primary"):
            try:
                ...
                if 'graph_data' in st.session_state and st.session_state.graph_data is not None:
                    ...
                    # If script was generated but graph data is missing, try to generate it
                    with st.spinner("Generating knowledge graph data..."):
                        graph_data = knowledge_graph.visualize_graph()
                        st.session_state.graph_data = graph_data
                        st.info("Generated knowledge graph from existing data")
                else:
                    st.error("Please generate a script first to populate the knowledge base")
                    st.stop()
                
                # Only continue if we have graph data
                if not graph_data or not isinstance(graph_data, dict) or 'nodes' not in graph_data:
                    st.error("Knowledge graph data is empty or invalid. Try generating a script first.")
                    st.stop()
                ...
                with st.spinner("Generating knowledge graph visualization..."):
                    ...
                    fig = visualize_knowledge_graph(graph_data)
                    st.pyplot(fig)
                    
                    # Add a note about interaction
                    st.info("You can hover over the graph to see node details.")
                ...
                with st.expander("About Knowledge Graphs"):
                    ...
                    - **Entities**: Key concepts, people, places or things (purple)
...
# Create a simple visualization function that doesn't rely on HTML/JS
def visualize_knowledge_graph(graph_data):
    """Create a simple matplotlib visualization of the knowledge graph"""
    ...
    for node in graph_data['nodes']:
        node_id = node['id']
        label = node.get('label', 'Unnamed')[:30]  # Limit label length
    ...
    # Draw edges
    nx.draw_networkx_edges(G, pos, width=1, alpha=0.5, edge_color='#9CA3AF')
    
    # Add labels to nodes, but only for entities to avoid clutter
    entity_labels = {node_id: G.nodes[node_id]['label'] for node_id in entity_nodes}
    nx.draw_networkx_labels(G, pos, labels=entity_labels, font_size=8, font_color='white')
